...
total_mining_by_program = 0
total_mining_by_remote = 0
...
def fast_step_exchange_by_multi_thread(currency_pair, accounts, target_coin='', base_coin=''):
    ...
    while True:
        ...
        if is_safe_wide(ticker.sellPrice, ticker.buyPrice):
            ...
            log(u'挖矿状态：%s' % order_services[buy_index].mining_difficult())
            log(u'程序挖矿预计: %s CARD' % (accumulated_amount * Decimal('0.002')))
...
def mining_across_btc_eth_channel(btc_currency_pair, btc_base_coin, eth_currency_pair, eth_base_coin, accounts, target_coin=''):
    # currency_pair, accounts, target_coin='', base_coin=''):
    if not (isinstance(accounts, list) or isinstance(accounts, tuple)):
        accounts = [accounts]
    # init market service
    market_service = CoinExMarketService()
    ...
    while True:
        # btc
        ticker = market_service.get_ticker(btc_currency_pair)
        if is_safe_wide(ticker.sellPrice, ticker.buyPrice):
            # 设置买卖账户下标
            sell_index = (inner_circulation_count + 0) % len(accounts)
            buy_index = (inner_circulation_count + 1) % len(accounts)
            ...
            # 最大操作量
            max_count = max_can_deal_num(target_coin, btc_base_coin, price, account_services[sell_index], account_services[buy_index])
            # 下单数量
            deal_count = (max_count * Decimal(str(round(random.uniform(0.600, 0.920), 3)))).quantize(Decimal('0.000'), rounding=ROUND_DOWN)
            # 特定币种有单笔交易量规定，如：不低于两百，小数不超过3位
            if deal_count < 200:
            ...
            sell_order.is_limit_order = True
            sell_order.is_sell_order = True
            sell_order.pair = btc_currency_pair
            t_sell = threading.Thread(target=order_services[sell_index].sell, args=(sell_order,), name=accounts[sell_index].name)
            ...
            buy_order.is_limit_order = True
            buy_order.is_sell_order = False
            buy_order.pair = btc_currency_pair
            t_buy = threading.Thread(target=order_services[buy_index].buy, args=(buy_order,), name=accounts[buy_index].name)
        ...
        if is_safe_wide(ticker.sellPrice, ticker.buyPrice):
            ...
            # 最大操作量
            max_count = max_can_deal_num(target_coin, eth_base_coin, price, account_services[sell_index], account_services[buy_index])
            # 下单数量
            deal_count = (max_count * Decimal(str(round(random.uniform(0.600, 0.920), 3)))).quantize(Decimal('0.000'), rounding=ROUND_DOWN)
            # 特定币种有单笔交易量规定，如：不低于两百，小数不超过3位
            if deal_count < 200:
            ...
            sell_order.is_limit_order = True
            sell_order.is_sell_order = True
            sell_order.pair = eth_currency_pair
            t_sell = threading.Thread(target=order_services[sell_index].sell, args=(sell_order,), name=accounts[sell_index].name)
            ...
            buy_order.is_limit_order = True
            buy_order.is_sell_order = False
            buy_order.pair = eth_currency_pair
            t_buy = threading.Thread(target=order_services[buy_index].buy, args=(buy_order,), name=accounts[buy_index].name)
...
if __name__ == '__main__':
    ...
    base_btc = 'BTC'
    base_eth = 'ETH'
    PAIR_card_btc = 'CARDBTC'
    PAIR_card_eth = 'CARDETH'
